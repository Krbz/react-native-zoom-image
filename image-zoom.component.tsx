...
import { container } from './image-zoom.style'
import {
  ICenterOn,
  IInitialValues,
  ImageZoomProps,
...
> {
  ...
  private imagePanResponder = PanResponder.create({
    ...
    onPanResponderTerminationRequest: this.props
      ...
      if (evt.nativeEvent.changedTouches.length <= 1) {
        if (
          ...
          if (this.props.enableDoubleClickZoom) {
            ...
            this.imageDidMove(OnMoveTypes.centerOn)
    ...
    onPanResponderMove: (evt, gestureState) => {
      ...
      this.imageDidMove(OnMoveTypes.onPanResponderMove)
    },
  ...
  public panResponderReleaseResolve = (): void => {
    ...
    this.imageDidMove(OnMoveTypes.onPanResponderRelease)
  }
  ...
  public componentDidMount(): void {
    ...
    if (this.props.initValues) {
      this.onInitialPosition(this.props.initValues)
    }
  }

  public onInitialPosition(initValues: IInitialValues): void {
    this.scale = initValues.scale
    this.animatedScale.setValue(this.scale)
    this.positionX = initValues.positionX
    this.animatedPositionX.setValue(this.positionX)
    this.positionY = initValues.positionY
    this.animatedPositionY.setValue(this.positionY)
  }
  ...
  public componentDidUpdate(prevProps: ImageZoomProps): void {
    ...
    if (this.props.initValues && !prevProps.initValues) {
      this.onInitialPosition(this.props.initValues)
    }
    ...
    if (
      ...
      prevProps.initValues.type !== this.props.initValues.type
    ) {
      this.onInitialPosition(this.props.initValues)
    }
  }

  public imageDidMove(type: OnMoveTypes): void {
  ...
  componentWillUnmount() {
    this.imageDidMove(OnMoveTypes.onUnmount)
  }
